...
// Upstream strips ask_user_question (ToolKind::AskUser) and the workflow tool from
// every subagent (xai-grok-agent/src/builder.rs, PromptAudience::Subagent), so the
// grok-agent allow-list below deliberately omits ask_user_question.
...
export const SUBAGENT_RECIPES = {
  'grok-agent': {
    ...
    toolFilter: { allow: ['run_terminal_cmd', 'read_file', 'search_replace', 'list_dir', 'grep', 'web_search', 'web_fetch', 'todo_write', 'task', 'get_task_output', 'kill_task', 'enter_plan_mode', 'exit_plan_mode'] },
  },
  'grok-explore': {
