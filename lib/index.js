...
// grok-build honors GROK_HOME to relocate its whole state dir (default ~/.grok).
// Honor it here too, and default the outbound proxy to a DIRECT connection —
// a hardcoded local proxy port breaks machines that do not run one.
const GROK_HOME = process.env.GROK_HOME || pathNative.join(process.env.USERPROFILE || process.env.HOME || os.homedir(), '.grok')
const GROK_PROXY = process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy || ''
...
function loadGrokKey() {
  try {
    const auth = JSON.parse(readTextFile(pathNative.join(GROK_HOME, 'auth.json')))
...
async function apply(ctx, config = {}) {
  await ensureKernelMesh(ctx, 'dsh-kernel-grok')
    const fs = ctx.get('fs')
    ...
    // grok's built-in subagent types map onto the single source of truth in
    // lib/subagents.js (upstream grok-build subagent_prompt.md + task.rs
    // GENERAL_PURPOSE/EXPLORE/PLAN_PROMPT). The recipe's persona is set
    // EXPLICITLY on every request because DSH's continuable (background)
    // route never invokes provider.start() — the continuation manager rebuilds
    // the child from the request fields recorded in the durable descriptor.
    // toolFilter is deliberately NOT set on the request: since dsh-tools
    // 0.1.1-rc.2, tools.restrict() accepts only GLOBAL tool names and rejects
    // scope-local (vendor) names; the mesh agent/created listener applies the
    // child tool mask instead (mesh AGENTS.md §6).
    const GROK_TYPE_TO_RECIPE = { 'general-purpose': 'grok-agent', explore: 'grok-explore', plan: 'grok-plan' }
    register(strDef({
      name: 'task',
      ...
      execute: async (args, exec) => {
        ...
        // Map grok's built-in subagent types onto the upstream recipes.
        const recipeName = GROK_TYPE_TO_RECIPE[args.subagent_type || 'general-purpose'] || 'grok-agent'
        const recipe = SUBAGENT_RECIPES[recipeName]
        let providerName = names.indexOf(recipeName) >= 0 ? recipeName : (names.indexOf('grok-agent') >= 0 ? 'grok-agent' : (names.indexOf('spawn') >= 0 ? 'spawn' : null))
        ...
        // No toolFilter on the request (see the note above GROK_TYPE_TO_RECIPE);
        // persona/agentOptions/maxDepth are the fields the durable descriptor
        // records for cold resume.
        const request = {
          label,
    ...
    if (systemPrompt) {
      ...
      // session on this kernel sees ONLY the upstream Grok Build prompt.
