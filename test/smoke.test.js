...
function createHarness() {
  ...
  const subagents = {
    list() {
      ...
      return ['grok-agent', 'grok-explore', 'spawn']
    },
    async startContinuable(req) {
      calls.startContinuable.push(req)
      return { childId: 'grok-child-1' }
    },
    async start(provider, request) {
      ...
      return {
        result: Promise.resolve({
          ...
          output: [{ type: 'text', text: 'grok partial answer' }],
        }),
        async dispose() {},
      }
    },
    ...
    async listChildren(parentId, signal) {
      ...
      return [{ id: 'grok-child-1', label: 'demo', mode: 'continuable' }]
    },
  }
...
const EXPECTED_TOOLS = [
  'run_terminal_cmd', 'read_file', 'search_replace', 'list_dir', 'grep',
  'web_search', 'web_fetch', 'todo_write', 'get_task_output', 'kill_task',
  'ask_user_question', 'task', 'enter_plan_mode', 'exit_plan_mode',
]

async function main() {
  const plugin = pluginMod
  ok(plugin, 'plugin module loads')
  eq(plugin.name, 'dsh-kernel-grok', 'plugin.name')
  ...
  const task = h.registered.get('task')
  ok(typeof task.isConcurrencySafe === 'function', 'task.isConcurrencySafe is a function')
  eq(task.isConcurrencySafe(), true, 'task.isConcurrencySafe() === true')

  const bg = await task.execute({
  ...
  eq(h.calls.followup[0].childId, 'grok-child-1', 'followup child id')
  eq(h.calls.followup[0].blocks[0].text, 'continue please', 'followup prompt text')
  match(resumed, /grok-msg-1/, 'resume text mentions message id')

  const fg = await task.execute({
    description: 'wait',
    prompt: 'finish this',
  ...
  const created = await search_replace.execute({
    ...
    old_string: '',
    new_string: 'brand new file',
  }, exec)
  eq(h.calls.writeText.length, 1, 'empty old_string takes the writeText path')
  eq(h.calls.editText.length, 0, 'empty old_string does not call editText')
  eq(h.readSeed('created.txt'), 'brand new file', 'search_replace write path persisted content')
  match(created, /written/, 'search_replace empty-old_string confirmation')

  const edited = await search_replace.execute({
    file_path: 'created.txt',
    old_string: 'brand new',
    new_string: 'updated',
  }, exec)
  eq(h.calls.editText.length, 1, 'non-empty old_string uses editText')
  eq(h.readSeed('created.txt'), 'updated file', 'search_replace edit path updated content')
  match(edited, /edited/, 'search_replace edit confirmation')

  const run_terminal_cmd = h.registered.get('run_terminal_cmd')
  const sh = await run_terminal_cmd.execute({ command: 'echo hi', description: 'spot-check shell' }, exec)
  eq(h.calls.spawn.length, 1, 'run_terminal_cmd foreground uses subprocess.spawn')
  match(sh, /\[exit code: 0\]/, 'run_terminal_cmd reports exit code')

  const { extractGrokSearch, htmlToText } = plugin._test
  const rendered = extractGrokSearch({
    output: [{
      type: 'message',
      content: [{
        type: 'output_text',
        text: 'Codex 0.147.0 is the latest stable.',
        annotations: [{ type: 'url_citation', title: 'Releases', url: 'https://github.com/openai/codex/releases' }],
      }],
    }],
  })
  match(rendered, /Codex 0\.147\.0/, 'grok search keeps synthesis text')
  match(rendered, /\[Releases\]\(https:\/\/github.com\/openai\/codex\/releases\)/, 'grok search lists citation links')
  eq(htmlToText('<html><style>a{}</style><p>Hi&amp;bye</p></html>'), 'Hi&bye', 'htmlToText strips tags')

  console.log('dsh-kernel-grok smoke: ' + assertionCount + ' assertions ok')
}
